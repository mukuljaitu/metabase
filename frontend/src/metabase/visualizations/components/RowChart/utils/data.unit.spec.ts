...
const countMetricColumn = createMockColumn({ name: "count" });
const avgMetricColumn = createMockColumn({ name: "avg" });
...
describe("data utils", () => {
  ...
  describe("chart with a breakout", () => {
    it("should group dataset by dimension values and breakout", () => {
      const groupedData = getGroupedDataset(dataset, breakoutChartColumns);
